...
def display_summary(data: dict[str, Any]) -> None:
    ...
    if very_simple > 0:
        ...
        print(
            f"  非常簡單        {very_simple:3d} 個 ({completion:3d} 完成, {rate:5.1f}%)"
        )
    ...
    if complex_count > 0:
        ...
        print(
            f"  複雜          {complex_count:3d} 個 ({completion:3d} 完成, {rate:5.1f}%)"
        )
    ...
    if very_complex > 0:
        ...
        print(
            f"  非常複雜        {very_complex:3d} 個 ({completion:3d} 完成, {rate:5.1f}%)"
        )


def display_template_list(
    data: dict[str, Any], status_filter: Optional[str] = None
) -> None:
    """顯示模板列表"""
    templates = data.get("templates", {})
    ...
    if status_filter:
        filtered = {
            k: v for k, v in templates.items() if v.get("status") == status_filter
        }
    else:
        filtered = templates
    ...
    # 排序：依複雜度
    sorted_templates = sorted(filtered.items(), key=lambda x: x[1].get("complexity", 0))
    ...
    for name, info in sorted_templates:
        ...
        print(
            f"{name:<45s} {status_display:<12s} {complexity:<8d} {dependencies:<8d} {conflicts:<6d}"
        )
...
範例：
  ...
  python standalone_migration_status.py --list
    ...
    parser.add_argument("--list", action="store_true", help="顯示詳細的模板列表")
