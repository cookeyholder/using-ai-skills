...
from migration_tracker import MigrationStatusTracker

def main():
    parser = argparse.ArgumentParser(description='Generate migration status snapshot.')
    parser.add_argument('-s', '--snapshots', type=str, default='snapshots',
    ...
    print(f"Initializing MigrationStatusTracker...")
    try:
                ...
                with open(output_path, 'r', encoding='utf-8') as f:
                    ...
                    for name, record in tracker.migration_status.items():
                        if name in existing_templates:
                            ...
                            record['status'] = existing_record.get('status', 'not_started')
