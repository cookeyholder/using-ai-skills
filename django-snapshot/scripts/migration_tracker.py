...
class MigrationStatusTracker:
    ...
    def calculate_complexity_score(
        self, template_name: str, css_data: Dict[str, Any]
    ) -> int:
        """
        計算模板的遷移複雜度評分（1-10）。
        ...
        for category_data in bootstrap_templates.values():
            if isinstance(category_data, dict):
                for templates_list in category_data.values():
                    if (
                        isinstance(templates_list, list)
                        and template_name in templates_list
                    ):
                        bootstrap_count += 1
        ...
        dependency_score = min(
            3, len(self.dependency_graph.get(template_name, [])) // 3
        )

        score = min(
            10, 1 + bootstrap_score + custom_score + icon_score + dependency_score
        )
        return max(1, score)
    ...
    def get_dependency_depth(
        self, template_name: str, visited: Optional[Set[str]] = None
    ) -> int:
        """
        計算模板的依賴深度（遞迴）。
    ...
    def update_status(
        self, template_name: str, status: str, notes: str = "", user: str = "system"
    ) -> bool:
        """
        更新模板的遷移狀態。
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        not_started = sum(
            1
            for r in self.migration_status.values()
            if r["status"] == self.STATUS_NOT_STARTED
        )
        in_progress = sum(
            1
            for r in self.migration_status.values()
            if r["status"] == self.STATUS_IN_PROGRESS
        )
        completed = sum(
            1
            for r in self.migration_status.values()
            if r["status"] == self.STATUS_COMPLETED
        )
        ...
        return {
            ...
            "templates_with_conflicts": sum(
                1 for r in self.migration_status.values() if r["conflicts"]
            ),
        }
